...
const NotificationSettings = () => (
  <div className="settings-grid">
    <SettingsCard title="이상 징후 알림">
      <SettingRow
        label="위험 즉시 알림"
        description="위험 이상 징후 감지 시 상단 알림과 toast를 표시합니다."
        enabled
      />
      <SettingRow
        label="Warning 묶음 알림"
        description="주의 단계 이벤트는 10분 단위로 묶어서 표시합니다."
        enabled
      />
      <SettingRow
        label="참고 알림"
        description="참고 이벤트는 기본적으로 목록에만 표시합니다."
      />
    </SettingsCard>
    <SettingsCard title="전송 채널">
    ...
    <SettingsCard title="알림 시간">
      ...
      <SettingRow
        label="야간 알림 축소"
        description="야간에는 위험과 보류 변경만 표시합니다."
        enabled
      />
    </SettingsCard>
  </div>
...
const DisplaySettings = () => (
  <div className="settings-grid">
    ...
    <SettingsCard title="홈 위젯">
      <SettingRow
        label="위젯 편집 애니메이션"
        description="macOS 스타일 편집 흔들림과 drag 전환을 사용합니다."
        enabled
      />
      <SettingRow
...
const ChannelRow = ({ label, status }: { label: string; status: string }) => (
  <div className="settings-channel-row">
    <span>{label}</span>
    <Badge variant={status === "사용 중" ? "success" : "default"}>
      {status}
    </Badge>
  </div>
);
